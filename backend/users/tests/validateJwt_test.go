...
func TestValidateJWT(t *testing.T) {
	...
	testCases := []testCase{
		{
			...
			tokenString: generateValidTokenJWTValidationTest(t, issuer, secretKey),
			wantErr:     false,
			wantClaims: jwt.MapClaims{
				"iss": issuer,
		...
		},
		{
			...
			tokenString: generateValidTokenJWTValidationTest(t, issuer, "another-secret"),
			wantErr:     true,
		},
		{
	...
	for _, tt := range testCases {
		t.Run(tt.description, func(t *testing.T) {
			gotClaims, err := lib.ValidateJWT(tt.tokenString)
...
func generateValidTokenJWTValidationTest(t *testing.T, issuer, key string) string {
	now := time.Now()
